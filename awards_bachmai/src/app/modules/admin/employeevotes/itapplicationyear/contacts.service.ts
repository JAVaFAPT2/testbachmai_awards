...
export class ContactsService {
    private _userVoteCount = new BehaviorSubject<number>(0);

    // Add getter for vote count
    get userVoteCount$(): Observable<number> {
        return this._userVoteCount.asObservable();
    }
    toggleVote(contact: Contact): Observable<Contact> {
        return this._userVoteCount.pipe(
            take(1),
            switchMap(currentVotes => {
                if (contact.hasVoted) {
                    // Unvote
                    this._userVoteCount.next(currentVotes - 1);
                    contact.hasVoted = false;
                    contact.votes--;
                } else if (currentVotes < 5) {
                    // Vote
                    this._userVoteCount.next(currentVotes + 1);
                    contact.hasVoted = true;
                    contact.votes++;
                }
